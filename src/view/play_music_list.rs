...
pub fn draw_play_music_list<B: Backend>(
    ...
    frame.render_widget(block, area); //渲染部件

    if music_list.len() > 0 || playing_music != &None {
        draw_play_list(
            frame,          //frame: &mut Frame<B>,
            &area,          //area: &Rect,
            theme,          //theme: &Theme,
            music_list,     //music_list: &Vec<Music>,
            &playing_music, //playing_music: &Option<Music>,
...
fn draw_play_list<B: Backend>(
    ...
    if let Some(playing_music) = playing_music {
        ...
        names.push(Spans::from(vec![
            ...
            Span::styled(
                //A string where all graphemes have the same style.
                format!("{:>3}", lable),
                Style::default().fg(theme.gauge_label_color), //仪表标签颜色
            ),
            Span::styled(
            ...
            Span::styled(
                ...
                Style::default()
                    ...
                    .add_modifier(Modifier::BOLD), //Changes the text emphasis.
            ),
        ]));
        artists.push(Spans::from(vec![Span::styled(
            format!(" {}", &playing_music.artist),
            Style::default().fg(theme.play_music_list_artist_color), //color.rs
        )]));
    ...
    frame.render_widget(Paragraph::new(names), chunks[0]); //使用 Widget::render 将 Widget 渲染到当前缓冲区。
