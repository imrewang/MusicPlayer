...
pub fn draw_music_list<B: Backend>(
    ...
    let display = Display::new(
        window_height,  //height: usize
        files.len(),    //content_len: usize
        selected_index, //selected_index: usize
    );
    ...
    let block = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded) //边框类型
            ...
            match &files[i] {
                DirectoryItem::File(path) => {
                    ...
                    music_names.push(get_spans(
                        " 🎷 ".to_string(),   //icon: String
                        name.to_string(),       //name: String
                        theme.list_icon_color,  // icon_color: Color
                        theme.list_music_color, //name_color: Color
                    ));
                }
    ...
    let chunks = Layout::default()
        ...
        .constraints([Constraint::Length(3), Constraint::Min(3)]) //约束
        .split(inner_rect);
